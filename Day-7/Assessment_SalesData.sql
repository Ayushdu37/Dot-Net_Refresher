...
-- Normalization Problems Identified:
-- 1. 1NF Violation: Non-atomic multi-valued attributes (ProductNames, Quantities, UnitPrices contain comma-separated values).
-- 2. 2NF/3NF Violation: Partial and transitive dependencies (Customer details like CustomerPhone and CustomerCity depend on CustomerName, not OrderID).
-- 3. Inappropriate Data Types: OrderDate is VARCHAR(20) instead of DATE, quantities/prices stored as text.
...
-- Question 5: String & Date Functions (Uppercase name, month, filter Jan 2026 / Jan 2024)
SELECT 
    UPPER(CustomerName) AS CustomerName_Uppercase,
    MONTH(TRY_CAST(OrderDate AS DATE)) AS OrderMonth,
    OrderDate
FROM Sales_Raw
WHERE TRY_CAST(OrderDate AS DATE) >= '2024-01-01' AND TRY_CAST(OrderDate AS DATE) <= '2024-01-31';
